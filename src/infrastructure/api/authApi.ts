...
export async function verifySessionRequest(): Promise<User | null> {
...
export async function devLoginRequest(): Promise<User | null> {
  ...
  if (!response.ok) {
    console.warn(
      `[AuthContext] dev-login unavailable (status ${response.status}) -- ` +
